...
public class CompanyController {
    ...
    public ResponseEntity<CompanyWaitlistEmployees> getCompanyWaitlistEmployee(
            @RequestHeader("Authorization") String authHeader
    ){
        String hrId = jwtHelper.getUserIdFromToken(authHeader.substring(7));
        CompanyWaitlistEmployees waitlistEmployeesId = companyServiceInterface.getWaitlistEmployeesId(hrId);
